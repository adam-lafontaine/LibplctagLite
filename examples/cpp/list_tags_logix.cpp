...
bool get_udt_definition(plc::TagAttr& tag_attr, uint16_t udt_id);
...
int main(int argc, char** argv)
{
    ...
    if (tag_string_base == NULL) {
        tag_print_error("Unable to set up the tag string base!\n");
        usage();
    }*/
    ...
    plc::TagAttr tag_attr{};
    tag_attr.controller = plc::Controller::ControlLogix;
    tag_attr.gateway = "192.168.19.230"; // argv[1];
    tag_attr.path = "1,0"; // argv[2];
    tag_attr.tag_name = "@tags";

    /* set up the tag for the listing first. */
    //auto controller_result = open_tag(tag_string_base, "@tags");
    auto controller_result = plc::connect(tag_attr);
    if (!controller_result.is_ok())
    {
        tag_print_error("Unable to create listing tag, error %s!\n", controller_result.error);
        usage();
    }
    ...
    if (!rc) {
        tag_print_error("Unable to get tag list or no tags visible in the target PLC!\n");
        usage();
    }

    tag_print("Controller tag listing tag ID: %d\n", controller_listing_tag);
    ...
    for (struct tag_entry_s* entry = tag_list; entry; entry = entry->next) {
        if (strncmp(entry->name, "Program:", strlen("Program:")) == 0) {
            ...
            tag_print("Getting tags for program \"%s\".\n", entry->name);

            snprintf(buf, sizeof(buf), "%s.@tags", entry->name);

            tag_attr.tag_name = buf;

            auto program_result = plc::connect(tag_attr);
            ...
            tag_print("Program tag listing tag ID: %d\n", program_listing_tag);

            rc = get_tag_list(program_result.data, &tag_list, entry);
            if (!rc) {
                tag_print_error("Unable to get program tag list or no tags visible in the target PLC!\n");
                usage();
            }

            plc::destroy(program_listing_tag);
            tag_print("Destroying program tag listing tag ID: %d\n", program_listing_tag);
        }
    }
    ...
    for (struct tag_entry_s* entry = tag_list; entry; entry = entry->next) {
        ...
        if ((element_type & TYPE_IS_STRUCT) && !(element_type & TYPE_IS_SYSTEM)) {
            ...
            if (last_udt >= MAX_UDTS) {
                ...
                tag_print_error("More than %d UDTs are requested!\n", MAX_UDTS);
                usage();
            }
        }
    }
    ...
    while (current_udt < last_udt) {
        ...
        if (udts[udt_id] == NULL) {
            rc = get_udt_definition(tag_attr, udt_id);
            if (!rc) {
                plc::destroy(controller_listing_tag);
                tag_print_error("Unable to get UDT template ID %u!\n", (unsigned int)(udt_id));
                usage();
            }
        }
        else {
            tag_print_error("Already have UDT (%04x) %s.\n", (unsigned int)udt_id, udts[udt_id]->name);
        }
    ...
    for (struct tag_entry_s* tag = tag_list; tag; tag = tag->next) {
        if (!tag->parent) {
            tag_print("Tag \"%s", tag->name);
        }
        else {
            tag_print("Tag \"%s.%s", tag->parent->name, tag->name);
        }

        switch (tag->num_dimensions) {
        case 1:
            tag_print("[%d]", tag->dimensions[0]);
            break;

        case 2:
            tag_print("[%d,%d]", tag->dimensions[0], tag->dimensions[1]);
            break;

        case 3:
            tag_print("[%d,%d,%d]", tag->dimensions[0], tag->dimensions[1], tag->dimensions[2]);
            break;
        ...
        if (!tag->parent) {
            tag_print("tag string = \"protocol=ab-eip&gateway=%s&path=%s&plc=ControlLogix&elem_size=%u&elem_count=%u&name=%s\"\n", host, path, tag->elem_size, tag->elem_count, tag->name);
        }
        else {
            tag_print("tag string = \"protocol=ab-eip&gateway=%s&path=%s&plc=ControlLogix&elem_size=%u&elem_count=%u&name=%s.%s\"\n", host, path, tag->elem_size, tag->elem_count, tag->parent->name, tag->name);
        }
    }
    ...
    for (int index = 0; index < MAX_UDTS; index++) {
        ...
        if (udt) {
            if (udt->name) {
                tag_print(" UDT %s (ID %x, %d bytes, struct handle %x):\n", udt->name, (unsigned int)(udt->id), (int)(unsigned int)udt->instance_size, (int)(unsigned int)udt->struct_handle);
            }
            else {
                tag_print(" UDT *UNNAMED* (ID %x, %d bytes, struct handle %x):\n", (unsigned int)(udt->id), (int)(unsigned int)udt->instance_size, (int)(unsigned int)udt->struct_handle);
            }
            for (int field_index = 0; field_index < udt->num_fields; field_index++) {
                if (udt->fields[field_index].name) {
                    tag_print("    Field %d: %s, offset %d", field_index, udt->fields[field_index].name, udt->fields[field_index].offset);
                }
                else {
                    tag_print("    Field %d: *UNNAMED*, offset %d", field_index, udt->fields[field_index].offset);
                }

                /* is it a bit? */
                if (udt->fields[field_index].type == 0xC1) {
                    /* bit type, the metadata is the bit number. */
                    tag_print(":%d", (int)(unsigned int)(udt->fields[field_index].metadata));
                }

                /* is it an array? */
                if (udt->fields[field_index].type & 0x2000) { /* MAGIC */
                    tag_print(", array [%d] of type ", (int)(unsigned int)(udt->fields[field_index].metadata));
                }
                else {
                    tag_print(", type ");
                }
...
void usage()
{
    tag_print_error("Usage: list_tags <PLC IP> <PLC path>\nExample: list_tags 10.1.2.3 1,0\n");
    if (out_file) { fclose(out_file); }
    if (err_file) { fclose(err_file); }
    exit(1);
}
...
//        tag_print_error("Hostname or IP address must not be zero length!\n");
//        usage();
//    }
//
//    gateway = argv[1];
...
//        tag_print_error("PLC path must not be zero length!\n");
//        usage();
//    }
//
//    path = argv[2];
...
//
//    /* FIXME - check size! */
//    tag_print_error("Using tag string \"%s\".\n", tag_string);
//
...
//    if (result.is_error())
//    {
//        tag_print_error("Unable to open tag!  Return code %s\n", result.error);
//        usage();
//    }
//
//    return result;
...
bool get_tag_list(plc::TagDesc const& tag, struct tag_entry_s** tag_list, struct tag_entry_s* parent)
{
    ...
    /* go get it. */
    auto result = plc::receive(tag.tag_handle);
    if (result.is_error())
    {
        tag_print_error("Error %s trying to send CIP request!\n", result.error);
        usage();
    }

    tag_print("Listing tag read, result of size %d.\n", payload_size);

    /* check the payload size. */
    if (payload_size < 4) {
        tag_print_error("Unexpectedly small payload size %d!\n", payload_size);
        usage();
    }
...
bool process_tag_entry(int32_t tag, int* offset, uint16_t* last_tag_id, struct tag_entry_s** tag_list, struct tag_entry_s* parent)
{
    ...
    *offset += 4;
    ...
    if (!tag_name) {
        tag_print_error("Unable to allocate memory for the tag name!\n");
        return false;
    }

    auto str_result =  plc::get_string(tag, *offset, tag_name, tag_name_len + 1);
    if (!str_result.is_ok()) {
        tag_print_error("Unable to get tag name string, error %s!\n", str_result.error);
        free(tag_name);
        return false;
    }
...
void print_element_type(uint16_t element_type)
{
    if (element_type & TYPE_IS_SYSTEM) {
        tag_print("element type SYSTEM %04x", (unsigned int)(element_type));
    }
    else if (element_type & TYPE_IS_STRUCT) {
        tag_print("element type UDT (0x%04x) %s", (unsigned int)(element_type), udts[(size_t)(unsigned int)(element_type & TYPE_UDT_ID_MASK)]->name);
    }
    else {
        ...
        if (type) {
            tag_print("(%04x) %s", (unsigned int)element_type, type);
        }
        else {
            tag_print("UNKNOWN TYPE %04x", (unsigned int)element_type);
        }
    }
}
...
bool get_udt_definition(plc::TagAttr& tag_attr, uint16_t udt_id)
{
    ...
    auto tag_result = plc::connect(tag_attr); // open_tag(tag_string_base, buf);
    if (tag_result.is_error())
    {
        if (tag_result.status == plc::Status::ERR_UNSUPPORTED)
        {
            tag_print_error("This PLC type does not support listing UDT definitions.\n");
            return false;
        }

        tag_print_error("Unable to open UDT info tag, error %s!\n", tag_result.error);
        usage();
    ...
    tag_print("UDT info tag ID: %d\n", udt_info_tag);

    auto result = plc::receive(udt_info_tag);
    if (result.status == plc::Status::ERR_UNSUPPORTED) {
        plc::destroy(udt_info_tag);
        tag_print_error("UDT tag introspection is not supported on this PLC.\n");
        return false;
    }
    else if (result.is_ok()) {
        tag_print_error("Error %s while trying to read UDT info!\n", result.error);
        usage();
        return false;
    }
    ...
    if (template_id != udt_id) {
        tag_print_error("The ID, %x, of the UDT we are reading is not the same as the UDT ID we requested, %x!\n", (unsigned int)template_id, (unsigned int)udt_id);
        usage();
    }
    ...
    if (!udts[(size_t)udt_id]) {
        tag_print_error("Unable to allocate a new UDT definition structure!\n");
        usage();
        return false;
    }
    ...
    tag_print("Offset after reading field descriptors: %d.\n", offset);
    ...
    {
        tag_print_error("Unexpected raw UDT name length: %d! Error: %s\n", name_len, len_result.error);
        return false;
    }
    ...
    if (!name_str) {
        tag_print_error("Unable to allocate UDT name string!\n");
        usage();
        return false;
    }
    ...
    if (!str_result.is_ok())
    {
        tag_print_error("Error %s retrieving UDT name string from the tag!\n", str_result.error);        
        usage();
        free(name_str);
        return false;
    }
    ...
    if (name_len == 0 || name_len >= 256) {
        tag_print_error("Unexpected UDT name length: %d!\n", name_len);
        free(name_str);
        return false;
    }
    ...
    tag_print("Getting data from UDT \"%s\".\n", udts[udt_id]->name);
    ...
    tag_print("Getting %d field names for UDT %s.\n", udts[udt_id]->num_fields, udts[udt_id]->name);
    tag_print("offset=%u, tag_size=%u.\n", offset, tag_size);
    ...
    for (field_index = 0; field_index < udts[udt_id]->num_fields && offset < tag_size; field_index++) {
        tag_print_error("Getting name for field %u.\n", field_index);
        ...
        if (!len_result.is_ok() || name_len >= 256) {
            plc::destroy(udt_info_tag);
            tag_print_error("Unexpected UDT field name length: %d! Error: %s\n", name_len, len_result.error);
            usage();
            // TODO
        }

        tag_print_error("The name for field %u is %u characters long.\n", field_index, name_len);
        ...
        if (name_len > 0) {
            ...
            if (!name_str) {
                plc::destroy(udt_info_tag);
                tag_print_error("Unable to allocate UDT field name string!\n");
                usage();
                continue;
            }

            tag_print_error("The string for field %u is at %p.\n", field_index, (void*)name_str);

            /* copy the name */
            str_result = plc::get_string(udt_info_tag, offset, name_str, name_len + 1);
            if (!str_result.is_ok()) {
                plc::destroy(udt_info_tag);
                tag_print_error("Error %s retrieving UDT field name string from the tag!\n", str_result.error);
                free(name_str);
                usage();
                continue;
            }
            ...
            tag_print_error("UDT field %d is not named.\n", field_index);
    ...
    /* sanity check */
    if (offset != tag_size) {
        tag_print_error("Processed %d bytes out of %d bytes.\n", offset, tag_size);
    }
    ...
    tag_print("Destroying UDT info tag: %d\n", udt_info_tag);
    plc::destroy(udt_info_tag);
